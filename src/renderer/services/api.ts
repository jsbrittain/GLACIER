...
const electronAPI = isElectron
  ? {
      ...
      checkWslConfig: () => window.electronAPI.checkWslConfig(),
      writeWslconfig: (cpu, mem) => window.electronAPI.writeWslconfig(cpu, mem),
      restartWsl: () => window.electronAPI.restartWsl(),
...
const httpAPI = {
  ...
  checkWslConfig: async () => httpDispatch('/api/check-wsl-config', 'POST', {}),
  writeWslconfig: async (cpu, mem) => httpDispatch('/api/write-wslconfig', 'POST', { cpu, mem }),
  restartWsl: async () => httpDispatch('/api/restart-wsl', 'POST', {}),
